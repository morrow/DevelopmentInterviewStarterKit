...
export const getEmailCharacterFrequency = (people)=>{
  ...
  /*
    Generate a character array from an string input, which is generated by concatenating all of the email properties
    of a given people data object (people). Map over each character in this character array and update frequency count
    of that character in a frequencies object. Return an array that can be printed to a table using React Table.
  */
...
export const getPossibleDuplicates = (people)=> {
  ...
  /*
    My first idea would be to use a levenshtein distance library to determine how similar the relevant properties are.
    However, for this case, I read in the instructions not to use any libraries for the core logic of de-duping, so I created and implemented
    a way to detect differences and typos by creating a keyboard matrix and computing distances between keystrokes to detect
    typos. Example: email@salesloft.com with an additional keystroke: emaqil@salesloft.com.
    Sample data is not similar enough to generate any duplicates, so have a look at test/utils/getPossibleDuplicates to see what I am able to detect with this.
  */
